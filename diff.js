...
async function diff (context, heroku) {
  try {
    const diffs = await a.object({
      files: diffFiles(heroku, context.args.app1, context.args.app2),
    ...
    const list = [...diffs.files, ...diffs.env, ...diffs.stack, ...diffs.bp, ...diffs.addons, ...diffs.features]
    cli.table(list, {
      columns: [
